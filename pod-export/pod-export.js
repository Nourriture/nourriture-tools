...
if(args["ename"]) {
    keyword = args["ename"];
    expr = '[ g.GTIN_CD | g <~ pod.gtin, lower(g.GTIN_NM) == "' + args["ename"] + '" && g.BSIN ]';
    console.log("Starting export for products with exact name: \"" + args["ename"] + "\" ...");
}
if(args["name"]) {
    keyword = args["name"];
    expr = '[ g.GTIN_CD | g <~ pod.gtin, lower(g.GTIN_NM) =~ "' + args["name"] + '" && g.BSIN ]';
    console.log("Starting export for products with name containing: \"" + args["name"] + "\" ...");
}
if(args["category"]) {
    keyword = args["category"];
    expr = '[ g.GTIN_CD | g <~ pod.gtin, lower(g.PRODUCT_LINE) =~ "' + args["category"] + '" && g.BSIN ]';
...
async.waterfall(
    [
        ...
        function(callback){
            mingleQuery({
                    expr: expr, // Dependant on cmd-line args (see above)
                    limit: 100
                },
                function(err, res) {
                    if(!err) {
        ...
        function(products, callback) {
            ...
            async.each(products,
                ...
                function(gtin, productDone) {
                    ...
                    var url = args["pichost"] + "gtin-" + group + "/" + gtin + ".jpg";
...
function(err, products, companies) {
    if(!err) {
        ...
        var productsPath = args["out"] + "/" + keyword + ".json";
        var companiesPath = args["out"] + "/" + keyword + "-companies.json";
        async.parallel([
